...
use freepst_library::foundations::NativeElement;
use freepst_library::introspection::{SplitLocator, Tag};
use freepst_library::layout::{Abs, Dir, Em, Fr, Frame, FrameItem, Point};
use freepst_library::model::{ParLine, ParLineMarker};
use freepst_library::text::{Lang, TextElem};
...
fn add_par_line_marker(
    ...
    let key = freepst_utils::hash128(&marker);
