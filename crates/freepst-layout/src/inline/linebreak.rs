...
use freepst_library::layout::{Abs, Em};
use freepst_library::model::Linebreaks;
use freepst_library::text::{is_default_ignorable, Lang, TextElem};
use freepst_syntax::link_prefix;
...
fn blob() -> BlobDataProvider {
    BlobDataProvider::try_new_from_static_blob(freepst_assets::icu::ICU).unwrap()
}
...
static CJ_SEGMENTER: LazyLock<LineSegmenter> = LazyLock::new(|| {
    let cj_blob =
        BlobDataProvider::try_new_from_static_blob(freepst_assets::icu::ICU_CJ_SEGMENT)
            .unwrap();
