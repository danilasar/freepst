...
use freepst_library::foundations::{Style, StyleChain};
use freepst_library::layout::{Abs, Em, FixedAlignment, Frame, Point, Size, VAlignment};
use freepst_library::math::{EquationElem, MathSize};
...
macro_rules! scaled {
    ($ctx:expr, $styles:expr, text: $text:ident, display: $display:ident $(,)?) => {
        match freepst_library::math::EquationElem::size_in($styles) {
            freepst_library::math::MathSize::Display => scaled!($ctx, $styles, $display),
    ...
    ($ctx:expr, $styles:expr, $name:ident) => {
        $crate::math::Scaled::scaled(
            $ctx.constants.$name(),
            $ctx,
            freepst_library::text::TextElem::size_in($styles),
        )
    };
}
