...
use freepst_library::diag::{bail, warning, SourceResult};
use freepst_library::engine::{Engine, Route, Sink, Traced};
use freepst_library::foundations::{Packed, Resolve, Smart, StyleChain};
...
use freepst_library::routines::{Pair, Routines};
...
impl<T> CachedCell<T> {
    ...
    where
        ...
        F: FnOnce(I) -> T,
    {
        let input_hash = freepst_utils::hash128(&input);
