use freepst_library::diag::{bail, error, At, SourceDiagnostic, SourceResult};
use freepst_library::foundations::{ops, IntoValue, Value};
use freepst_syntax::ast::{self, AstNode};
use freepst_syntax::{Span, SyntaxKind, SyntaxNode};
...
impl Eval for ast::WhileLoop<'_> {
    ...
    #[freepst_macros::time(name = "while loop", span = self.span())]
    fn eval(self, vm: &mut Vm) -> SourceResult<Self::Output> {
...
impl Eval for ast::ForLoop<'_> {
    ...
    #[freepst_macros::time(name = "for loop", span = self.span())]
    fn eval(self, vm: &mut Vm) -> SourceResult<Self::Output> {
        let flow = vm.flow.take();
        let mut output = Value::None;
