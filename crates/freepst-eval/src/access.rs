use ecow::eco_format;
use freepst_library::diag::{bail, At, Hint, SourceResult, Trace, Tracepoint};
use freepst_library::foundations::{Dict, Value};
...
pub(crate) fn access_dict<'a>(
    ...
    match access.target().access(vm)? {
        ...
        value => {
            ...
            } else if freepst_library::foundations::fields_on(ty).is_empty() {
