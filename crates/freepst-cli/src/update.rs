...
const TYPST_GITHUB_ORG: &str = "freepst";
const TYPST_REPO: &str = "freepst";
...
fn extract_binary_from_zip(data: &[u8], asset_name: &str) -> StrResult<Vec<u8>> {
    ...
    let mut file =
        archive.by_name(&format!("{asset_name}/freepst.exe")).map_err(|err| {
...
fn extract_binary_from_tar_xz(data: &[u8]) -> StrResult<Vec<u8>> {
    ...
    let mut file = archive
        ...
        .find(|e| e.path().unwrap_or_default().ends_with("freepst"))
