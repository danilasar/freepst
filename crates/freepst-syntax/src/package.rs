    ...
    pub fn validate(&self, spec: &PackageSpec) -> Result<(), EcoString> {
        ...
        if let Some(required) = self.package.compiler {
            ...
            if !current.matches_ge(&required) {
                return Err(eco_format!(
                    "package requires freepst {required} or newer \
...
fn parse_version(s: &mut Scanner) -> Result<PackageVersion, EcoString> {
    ...
    if !version.is_empty() {
        ...
        Ok(PackageVersion::make_undefined())
    }
}
...
impl PackageVersion {
    ...
    /// Генерирует экземпляр неизвестной версии
    pub fn make_undefined() -> Self {
        Self { major: 0, minor: 0, patch: 0 }
    }

    /// Проверяет, известна ли версия
    pub fn is_undefined(&self) -> bool {
        *self == Self::make_undefined()
    }
