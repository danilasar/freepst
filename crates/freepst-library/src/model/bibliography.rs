...
use freepst_syntax::{Span, Spanned};
use freepst_utils::{LazyHash, NonZeroExt, PicoStr};
...
impl Show for Packed<BibliographyElem> {
    #[freepst_macros::time(name = "bibliography", span = self.span())]
...
impl Bibliography {
    ...
    #[comemo::memoize]
    #[freepst_macros::time(name = "load bibliography")]
    fn load(paths: &BibliographyPaths, data: &[Bytes]) -> StrResult<Bibliography> {
        ...
        Ok(Bibliography {
            map: Arc::new(map),
            hash: freepst_utils::hash128(data),
        })
    }
