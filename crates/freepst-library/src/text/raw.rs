...
use freepst_syntax::{split_newlines, LinkedNode, Span, Spanned};
...
impl Packed<RawElem> {
    ...
    fn highlight(&self, styles: StyleChain) -> Vec<Packed<RawLine>> {
        ...
        let mut seq = vec![];
        if matches!(lang.as_deref(), Some("typ" | "freepst" | "typc" | "typm")) {
            let text =
                lines.iter().map(|(s, _)| s.clone()).collect::<Vec<_>>().join("\n");
            let root = match lang.as_deref() {
                Some("typc") => freepst_syntax::parse_code(&text),
                Some("typm") => freepst_syntax::parse_math(&text),
                _ => freepst_syntax::parse(&text),
            };
...
impl Show for Packed<RawElem> {
    #[freepst_macros::time(name = "raw", span = self.span())]
...
impl Show for Packed<RawLine> {
    #[freepst_macros::time(name = "raw.line", span = self.span())]
...
/// Wrapper struct for the state required to highlight freepst code.
...
impl<'a> ThemedHighlighter<'a> {
    ...
    fn highlight_inner(&mut self) {
        ...
        for child in self.node.children() {
            ...
            if let Some(tag) = freepst_syntax::highlight(&child) {
...
#[comemo::memoize]
#[freepst_macros::time(name = "load syntaxes")]
...
#[comemo::memoize]
#[freepst_macros::time(name = "load theme")]
...
pub static RAW_THEME: LazyLock<synt::Theme> = LazyLock::new(|| synt::Theme {
    ...
    scopes: vec![
        ...
        item("string.other.math.freepst", None, None),
