    ...
    fn resolve(&self) -> Resolved {
        match self.outer {
            ...
            Some(outer) => match outer.resolve() {
                Resolved::Hash(outer) => {
                    Resolved::Hash(freepst_utils::hash128(&(self.local, outer)))
                }
...
impl<'a> SplitLocator<'a> {
    ...
    pub fn next<K: Hash>(&mut self, key: &K) -> Locator<'a> {
        self.next_inner(freepst_utils::hash128(key))
    }
    ...
    pub fn next_inner(&mut self, key: u128) -> Locator<'a> {
        ...
        let local = freepst_utils::hash128(&(key, disambiguator, self.local));
