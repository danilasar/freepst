...
use comemo::Track;
use ecow::EcoString;
use freepst_library::diag::{bail, At, SourceResult};
use freepst_library::engine::Engine;
use freepst_library::foundations::{
...
use freepst_library::html::{tag, HtmlElem};
use freepst_library::introspection::{Locatable, SplitLocator, Tag, TagElem};
use freepst_library::layout::{
    AlignElem, BoxElem, HElem, InlineElem, PageElem, PagebreakElem, VElem,
};
use freepst_library::math::{EquationElem, Mathy};
use freepst_library::model::{
    CiteElem, CiteGroup, DocumentElem, EnumElem, ListElem, ListItemLike, ListLike,
    ParElem, ParbreakElem, TermsElem,
};
use freepst_library::routines::{Arenas, Pair, RealizationKind};
use freepst_library::text::{LinebreakElem, SmartQuoteElem, SpaceElem, TextElem};
use freepst_syntax::Span;
use freepst_utils::{SliceExt, SmallBitSet};

/// Realize content into a flat list of well-known, styled items.
#[freepst_macros::time(name = "realize")]
pub fn realize<'a>(
...
fn prepare(
    ...
    let key = freepst_utils::hash128(&target);
