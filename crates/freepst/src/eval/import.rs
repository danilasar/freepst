...
<<<<<<<< HEAD:crates/typst-eval/src/import.rs
    let manifest_id = FileId::new(Some(spec.clone()), VirtualPath::new("typst.toml"));
    let bytes = engine.world.file(manifest_id).at(span)?;
========
    let manifest_id = FileId::new(Some(spec.clone()), VirtualPath::new("freepst.toml"));
    let bytes = vm.world().file(manifest_id).at(span)?;
>>>>>>>> dbf12fc8 (Взлетаем):crates/freepst/src/eval/import.rs
