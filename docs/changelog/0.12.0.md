...
- Exported SVGs now contain the `data-freepst-label` attribute on groups resulting
...
## Development
- Added `freepst-kit` crate which provides useful APIs for `World` implementors
- Added go-to-definition API in `freepst-ide`
- Added package manifest parsing APIs to `freepst-syntax`
...
- Removed `Tracer` in favor of `Warned<T>` and `freepst::trace` function
